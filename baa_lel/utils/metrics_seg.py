...
def binary_surface_distance_metrics(pred_mask, target_mask):
    """Return symmetric HD95 and ASSD in 2-D pixel units.

    Both-empty masks have distance 0. If exactly one mask is empty, the image
    diagonal is used as a finite worst-case penalty so fold means remain defined.
    """
    pred = np.asarray(pred_mask, dtype=bool)
    target = np.asarray(target_mask, dtype=bool)
    if pred.shape != target.shape or pred.ndim != 2:
        raise ValueError("pred_mask and target_mask must be same-shape 2-D arrays")
    if not pred.any() and not target.any():
        return 0.0, 0.0
    if not pred.any() or not target.any():
        penalty = float(np.hypot(*pred.shape))
        return penalty, penalty

    pred_surface = pred ^ binary_erosion(pred, structure=np.ones((3, 3)), border_value=0)
    target_surface = target ^ binary_erosion(target, structure=np.ones((3, 3)), border_value=0)
    dist_to_target = distance_transform_edt(~target_surface)[pred_surface]
    dist_to_pred = distance_transform_edt(~pred_surface)[target_surface]
    distances = np.concatenate([dist_to_target, dist_to_pred])


    assd = 0.5 * (float(np.mean(dist_to_target)) + float(np.mean(dist_to_pred)))
    return float(np.percentile(distances, 95)), assd
