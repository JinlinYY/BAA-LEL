...
from bua_lel.models.boundary.anchor_boundary_graph import AnchorConstrainedBoundaryGraph


def test_complete_model_forward_without_external_weights():
    torch.set_num_threads(1)
    model=BUALEL(clinical_dim=2,numeric_slice=(0,2),onehot_slices_dict={},num_classes=3,image_size=64,medsam_checkpoint_path=None,freeze_medsam=True,unfreeze_medsam_last_n=0,late_raw_clinical_fusion=True).eval()
    with torch.no_grad():
